...
bool Booking() { //
    ...
    while(option < 1 || option > 3){
        ...
        if(option == 1) EditBooking(); //Edit Booking List
        else if(option == 2) AddBooking(); //Add New Guest Details into booking List
...
void OpenBooking(){//open booking list
    fstream inFile;
    string name, RT, tog, ci, co, email; //RT =room type, tog=type of guest, ci/co=check in/out 
    int pn; //pn=phone number

    cout << fixed << left 
         << setw(20) << "Name" << setw(20) << "Room type"//displaying header
    ...
    if(!inFile) cout << "Unable to open file" << endl;//incase wrong file is opened
    else{
        while(!inFile.eof()){
            inFile >> name >> RT 
                   >> tog >> ci >> co 
...
void EditBooking(){//edit booking list in the terminal 
    ...
    cout << "Enter name whose data is to be edited : "; //prompt user for name of student whose details are to be edited
    cin >> Ename; 
    cout << "Enter " << Ename << "'s new room type, check-in, check-out, email, phone number : \n"; 
    cin >> ERT >> Eci >> Eco >> Eemail >> Epn;//user enters new details
    ...
    if (!inFile || !outFile) cout << "Unable to open file" << endl;//incase wrong file is opened
        else { 
            while (!inFile.eof()){ 
                inFile >> name >> RT >> tog >> ci >> co >> email >> pn;
                if (inFile.fail()) break;//error handling
                if (name == Ename){ //updating original data with edited data
                    RT = ERT;
                    ci = Eci;
                    co = Eco;
                    email = Eemail;
                ...
                outFile  << left //updating the booking list with edited data
    ...
    printLine(100); 
    cout << setw(20) << "Name" << setw(20) << "Room type" //display header
         << setw(20) << "Type of student" << setw(20) << "Check-in" << setw(20) << "Check-out" 
         << setw(25) << "Email" << "Phone number" << endl;
    
    inFile.open("Booking List_temp.txt"); 
    if (!inFile) cout << "Unable to open file" << endl; //error hndling
    else{ 
        while (!inFile.eof()){ 
            inFile >> name >> RT >> tog >> ci >> co >> email >> pn; //input
            if (inFile.fail()) break;//error handling 
            cout << setw(20) << name << setw(20) << RT << setw(20) << tog << setw(20) << ci << setw(20) << co << setw(25) << email << pn << endl; //displaying edited booking list
        } 
    } 
    inFile.close(); 
    outFile.close(); 
...
void AddBooking(){//add new student and their data to the booking list
    ...
    cout << "Enter name, room type, type of guest, check-in, check-out, email, phone number : \n"; //prompt the user to input the new student's name and details
    cin >> name_add >> RT_add >> tog_add >> ci_add >> co_add >> email_add >> pn_add;//user enters new student's name and details
    cout << "Adding the following :\n" //for user to check that details entered are accurate
              << left 
              << name_add << "\t" << RT_add 
    ...
    if (!inFile ||!outFile) {cout << "Unable to open file\n"; }//incase wrong file is opened
    else{ 
        while (!inFile.eof()){ 
                inFile >> name >> RT >> tog >> ci >> co >> email >> pn;//read booking list
                if (inFile.fail()) break;//error handling
                outFile<< left 
                 << setw(20) << name << setw(20) << RT //updating booking list to include new student and their details
                 << setw(20) << tog << setw(20) << ci << setw(20) << co 
...
void SendBooking(){//sending data from booking list to guest allocations file
...
bool GuestRoom() {
    ...
    char hall; 
    int roomF = 0, floorF = 1, roomG = 0, floorG = 1; //F = Hall F, G = Hall G
	int option;
	bool LOOP;
    int floor,room;
    int reallocate;
    ifstream inFile;
    ofstream outFile;
    //printing Guest Room Allocation without allocating Room
    ...
    Rooms: 10 rooms per floor
    Hall F for Short Term Guests
    Hall G for Long Term Guests and University Students
    */
    if (reallocate == 1){
        system("CLS");
        printLine(100);
        ...
        cout << left<< setw(25) << "Name" << setw(20) << "Room Type" << setw(25) << "Type of Guest" << setw(20) << "Check-in" << setw(20) << "Check-out"
            << setw(30) << "Email" << setw(20) << "Phone Number" << "Hall, Floor,Room num" << endl;
        outFile << left;
            while (!inFile.eof()){//allocate guest room 
            inFile >> name >> roomtype >> typeofguest >> checkin >> checkout >> email >> phonenumber; //read data from Guest Room Allocation
            if (inFile.fail()) {break;}

            if (typeofguest == "Short-Term_Guest"){ //if guest is a Short Term Guest, he/she will be allocated to Hall F.
                hall = 'F';
                if (roomF >= 0 && roomF <= 10){roomF++;} //room avaliable per floor = 10
                else if (roomF > 10) { //if room > 10, move to a new floor, and reset room=1 
                    floorF++;
                    roomF = 1;
                }
                cout << setw(25) << name << setw(20) << roomtype << setw(25) << typeofguest << setw(20) << checkin << setw(20) << checkout << setw(30)
            ...
            else if (typeofguest == "Long-Term_Guest" || typeofguest == "University_Student"){ //if guest is a Long Term Guest or a University Student, he/she will be allocated to Hall F.
                hall = 'G';
                if (roomG >= 0 && roomG <= 10){roomG++;} //room avaliable per floor = 10
                else if (roomG > 10){ //if room > 10, move to a new floor, and reset room=1 
                    floorG++;
                    roomG = 1;
                }
                cout << setw(25) << name << setw(20) << roomtype << setw(25) << typeofguest << setw(20) << checkin << setw(20) << checkout << setw(30)
...
void SearchGuest(){
    ...
    inFile.open("Guest Room Allocation.txt");
        if (!inFile){cout << "Error opening file." << endl;}
            ...
            while (!inFile.eof()){   
                inFile >> name >> roomtype >> typeofguest >> checkin >> checkout >> email >> phonenumber >> hall >> floor >> room; //read data from Guest Room Allocation
                if (inFile.fail()) break;//error handling
                if (name == name_search){ //if name_search's name the same as the inFile's name, print the details of the guest
                    checkname = 1;
                    cout << left<< setw(25) << "Name" << setw(20) << "Room Type" << setw(25) << "Type of Guest" << setw(20) << "Check-in" << setw(20) << "Check-out"
                    << setw(30) << "Email" << setw(20) << "Phone Number" << "Hall, Floor,Room num" << endl;
                    cout << setw(25) << name << setw(20) << roomtype << setw(25) << typeofguest << setw(20) << checkin << setw(20) << checkout << setw(30)
...
void EditGuestRoom(){
    ...
    if (!inFile || !outFile){cout << "Unable to open file" << endl;}
        ...
        while (!inFile.eof()){
            inFile >> name >> roomtype >> typeofguest >> checkin >> checkout >> email >> phonenumber >> hall >> floor >> room; //input 
            if (inFile.fail()) break;//error handling 
            if (name == Ename){ //if name_search's name the same as the inFile's name, replace the details of the person with the Ehall,Efloor,Eroom
                hall = Ehall;
                floor = Efloor;
                room = Eroom;
    ...
    inFile.close();
    outFile.close();
    //User check to see if file is edited properly. Print Guest Room Allocation
    system("CLS");
    cout << "Below is the edited file\n";
    printLine(100);
    cout << left<< setw(25) << "Name" << setw(20) << "Room Type" << setw(25) << "Type of Guest" << setw(20) << "Check-in" << setw(20) << "Check-out"
...
bool BoarderDetails(){
    system("CLS");
    printLine(100);
    cout << "Boarder Details" << endl; //Header
    printLine(100);
    int option;
    bool LOOP = 1;
    ...
    ifstream inFile;
    inFile.open("Boarders Details.txt"); //Display Boarder Details
    if (!inFile){cout << "Unable to open file" << endl;}
    else{
        while (!inFile.eof()){
...
bool EditBoarder(){
    ...
    char Egender;
    bool checkname = 0, LOOP = 1, option;
    cout << "Enter name whose data is to be edited : ";
    cin >> Ename;
    ...
    outFile.open("temp_boarderdetails.txt", ofstream::out); //Temporary File to store edited details
    if (!inFile || !outFile){cout << "Unable to open file" << endl;}
    else{
        while (!inFile.eof()){
            inFile >> name >> gender >> level >> type >> nation >> school; //>> leave; //input
            if (inFile.fail()) break;//error handling
            if (name == Ename){ //replace all details after finding the name in the file
                gender = Egender;
                level = Elevel;
                type = Etype;
                nation = Enation;
...
bool RemoveBoarder(){
    ...
    ifstream inFile;
    ofstream outFile;
    inFile.open("Boarders Details.txt"); //Original File
    ...
    if (!inFile || !outFile){cout << "Unable to open file" << endl;}
        ...
        while (!inFile.eof()){
            inFile >> name >> gender >> level >> type >> nation >> school;//input
            if (inFile.fail()) break;//error handling
            if (name == Ename){ //replaces the data to be deleted into blank
                name = "";
                gender = "";
                level = "";
                type = "";
                nation = "";
                school = "";
                outFile << left << setw(24) << name << setw(8) << gender << setw(16) << level << setw(16) << type << setw(8) << nation << school << endl;//replaces data into blank
                checkname = 1; 
            }
            outFile << left << setw(24) << name << setw(8) << gender << setw(16) << level << setw(16) << type << setw(16) << nation << school << endl; 
        }
            ...
            if (!inFile) { cout << "Unable to open file\n"; }
                ...
                while (!inFile.eof()){
                    ...
                    if (Tusername == username){
                        Cusername = 1;//username is correct
                        if (Tpassword == password){
                            Cpassword = 1;//username and password is correct
                            break;
                        }
                        else{
                            Cpassword = 0;//username is correct,  but password is incorrect
                            break;
                        }
                    }
                    else Cusername = 0; //username not found/incorrect
                }
            }
            inFile.close();
        ...
        if (!inFile || !outFile){cout << "Unable to open file" << endl;}
            ...
            while (!inFile.eof()){
                ...
                if (Fusername == username){
                    Fusername = "";
                    Fpassword = "";
                    outFile << Fusername << Fpassword; //overwrites data into blank
                    findacc = 1;
                }
    ...
    if (LOGIN == 1){
        ...
        if (!inFile || !outFile){cout << "Unable to open file" << endl;}
            ...
            while (!inFile.eof()){
                ...
                if (inFile.fail()) break;//error handling
                if (username == currUSER){password = changedPW;}//change the password
...
bool BoarderRoom() {
    ...
    bool LOOP; 
    int reallocate;
    floor_aa = 2; //Initialise 1st Floor in Hall A as 2
    room_aa = 1; //Initialise 1st Room in Hall A as 1
    bed_aa = 64; //A-D 65-68, start with 64 in order to make sure the first bed is A after adding room number in the function

    floor_bb = 2;//Initialise 1st Floor in Hall B as 2
    room_bb = 1; //Initialise 1st Room in Hall B as 1
    bed_bb = 64; //A-D 65-68, start with 64 in order to make sure the first bed is A after adding room number in the function
    ...
    if (!inFile || !outFile) {cout << "Error opening file.";}
        ...
        if (reallocate == 1) {
            ...
            while (!inFile.eof()){ // read data and go through algorithms
                ...
                if(inFile.fail()) {break;}
                    if (gender_a[num_boarders] == 'M') { //if the boarder is a male student, allocate him into Hall A
                        hall = 'A'; 
                    ...
                    else if (gender_a[num_boarders] == 'F' ) {//if the boarder is a female student, allocate him into Hall B
                        hall = 'B';
                ...
                /*if(hall_a[num_boarders] > hall_a[num_boarders+1]) //sorting but unable to apply
                    {
                        char temp;
                        temp = hall_a[num_boarders];
                        hall_a[num_boarders] = hall_a[num_boarders+1];
...
//Search Boarder
    if (option == 1){
        ...
        cout << "Enter Name: ";
        cin >> name_search;
        for (int i = 0; i <= num_boarders;i++) { //check whether the name exists in the name_a array and if it does exist, continue code
        ...
        printLine(50);
        //EditBoarderRoom
        bool option_edit = 1; //if user wants to continue editing
                ...
                if (!inFile||!outFile) {cout << "Error opening file.";}
                    ...
                    while (!inFile.eof()) {                
                        ...
                        if (name_a[index] == name_a[index_found]){
                            hall_a[index] = hall_edit; //if name is found, replace the boarder's data with the input data
...
/*
Room Arrangements
Room - 1,2,6: 4 Beds Max
Room - 3,4,5: 2 Beds Max
Floor - 6 rooms
*/
int assign_room(char bed, int room) {
    if ((room == 1 || room == 2 || room == 6) && bed >= 68) {room++;}//if room =1,2 or 6, change room if 4 beds are used
    else if ((room >= 3 && room <= 5) && bed >= 66){room++;} //from room-3 to 5, change room if 2 beds are used
    else if (bed >= 64){}//continue if bed assigned has not reached D, continue in the same room
    else {cout << "Room Assignment Error";}
    return room;
}

char assign_bed(int room, char bed) {
    if (room == 1 || room == 2 || room == 6) { //if room =1,2 or 6
        if (bed >= 68) {bed = 65;} //if the previous bed assigned reached D, assign bed as A
        else {bed++;}
    }
    else if (room >= 3 && room <= 5) { //from room-3 to 5, change room if 2 beds are used and assign bed to A.
        if (bed >= 66) {bed = 65;} //if the previous bed assigned reached B, assign bed as A
        else {bed++;}  
    }
...
int assign_floor(int room, int floor) {
    if (room > 6) {floor++;}//if all 6 rooms are fully assigned, change to another floor
    else if (room >= 1 && room <= 6) {} //continue
    else {cout << "Floor Assignment Error";}
    return floor;
}